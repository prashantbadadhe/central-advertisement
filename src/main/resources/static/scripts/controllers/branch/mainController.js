myApp
		.controller(
				...
				[
						...
						'$timeout',
						'uiGridConstants',
						function($scope, $http, $uibModal, branchService, $filter, $interval, $timeout, uiGridConstants) {
							var self = this;
							self.branch = {
								id : null, name : '', addressLine2 : '', addressLine2 : '', country : '', state : '', city : '', zipcode : '', phone1 : '', email1 : '', company : ''
							}
							var gridApi;
							self.responeData = [];
							self.gridOptions = [];
							...
							$scope.pageName = 'Branch';
							$scope.pageUrl = 'branch';
							self.gridOptions = {
								...
								columnDefs : [
										{
											name : 'slno', width : 70, enableFiltering : false, cellClass : 'noborder', cellTooltip : 'Select to delete and dubble click to edit'

										},
										{
											field : 'name', displayName : 'Branch Name', cellClass : 'noborder', cellTooltip : 'Select to delete and dubble click to edit'
										...
										},
										{
											...
											field : 'phone1', displayName : ' Phones', cellClass : 'noborder', cellTemplate : '<span>{{row.entity.phone1}} <span ng-if = "row.entity.phone2.length!=0"> / </span>{{row.entity.phone2}}</span>', cellTooltip : 'Select to delete and dubble click to edit'

										}, {
											field : 'email1', displayName : ' Emails', cellClass : 'noborder', cellTemplate : '<span>{{row.entity.email1}}<span ng-if = "row.entity.email2.length!=0"> / </span> {{row.entity.email2}}</span>', cellTooltip : 'Select to delete and dubble click to edit'

										}, {
											field : 'company.name', displayName : 'Company Name', cellClass : 'noborder', cellTooltip : 'Select to delete and dubble click to edit'
							...
							self.deleteSelected = function() {
								angular.forEach(self.gridApi.selection.getSelectedRows(), function(data, index) {
									...
									branchService.deleteBranch(data).then(successCallback, errorCallback);
								});
							...
							self.openEditModal = function(rowDetails) {
								var modalInstance = $uibModal.open({
									templateUrl : '../partials/branchInputs.html', controller : 'BranchEditController', resolve : {
										items : function() {
							...
							self.openCreateModal = function() {
								var modalInstance = $uibModal.open({
									templateUrl : '../partials/branchInputs.html', controller : 'BranchCreateController'
							...
							self.getBranch = function() {
								...
								branchService.getBranches().then(successCallback, errorCallback);
								function successCallback(response) {
									self.pushGridData(response.data._embedded.branches)
								}
								function errorCallback(error) {
							...
							setSidebarMenuSelection("branch");
						} ]);